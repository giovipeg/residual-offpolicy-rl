#!/usr/bin/env python3
"""Smoke test for residual TD3 on CubeToContainerG1.

The G1 counterpart of `smoke_train_td3.py`. `train_residual_td3` has no CLI flag
for the base policy, so this calls `ensure_task_config.ensure()`, which generates
the `CubeToContainerG1` config class from giovi/configs/residual_td3_tasks.json,
writes the run id into it, and hands back the Hydra config name to launch with.

The argument is the *BC* run -- the one `smoke_train_act_g1.py` produced, whose
project is `dexmg-bc`.

Usage:
    python giovi/scripts/smoke_train_td3_g1.py dexmg-bc/<run_id>
    python giovi/scripts/smoke_train_td3_g1.py dexmg-bc/<run_id> --dry-run
    python giovi/scripts/smoke_train_td3_g1.py dexmg-bc/<run_id> algo.total_timesteps=1000
"""
...
OVERRIDES = [
    ...
    "eval_num_envs=1",
    "eval_num_episodes=1",
    "wandb.project=dexmg-cube-to-container-g1-td3",
    "debug=false",
]
